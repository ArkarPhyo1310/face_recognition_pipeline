...
class FaceFeaturesExtractor:
    def __init__(self):
        self.aligner = MTCNN(keep_all=True, device='cuda')
        self.facenet_preprocess = transforms.Compose([preprocessing.Whitening()])
        self.facenet = InceptionResnetV1(pretrained='vggface2').to('cuda').eval()
    ...
    def extract_features(self, img):
        ...
        embeddings = self.facenet(self.facenet_preprocess(faces).to('cuda')).cpu().detach().numpy()
